...
export default function Home() {
  ...
  useEffect(() => {
    if (!gameState) handleCreateGame(true);
  }, []);
  ...
  const handleCreateGame = async (isNewGame = true) => {
    ...
    try {
      const url = isNewGame
        ? `${API_URL}/api/game`
        : `${API_URL}/api/game/next-hand`;

      // ✅ ALWAYS send valid body for next hand
      ...
      if (!isNewGame) {
        ...
        requestBody = JSON.stringify({
          ...
          last_dealer_pos: gameState?.dealer ?? 0,
        });
      }
  ...
  const handlePlayerAction = async (action, amount = 0) => {
    ...
    try {
      const payload = { action, amount: amount || 0 };
  ...
  useEffect(() => {
    if (gameState) {
      ...
      if (gameState.winner) setShowPlayAgain(true);
    }
  }, [gameState]);

  useEffect(() => {
    if (gameState && gameState.current_player_id === 'bot' && !gameState.winner) {
      const timer = setTimeout(() => handlePlayerAction('none'), 1000);
  ...
  if (!gameState) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-gray-900">
        <h1 className="text-4xl text-white animate-pulse">Connecting to Game Server...</h1>
      </main>
    );
  }
          ...
          <div className="flex flex-col space-y-4 p-4 bg-gray-800/70 rounded-xl w-full max-w-sm">
            ...
            <div className="flex flex-col items-center">
              <input
                ...
                disabled={!isUserTurn}
              />
              <p className="font-mono mt-1 text-yellow-300 bg-black/30 px-3 rounded-full">
                ${raiseAmount}
              </p>
            </div>
          </div>
